...
function getFeaturedProjects($pdo) {
    try {
        $stmt = $pdo->query("SELECT * FROM projects WHERE is_featured = 1 ORDER BY created_at DESC");
        $projects = $stmt->fetchAll(PDO::FETCH_ASSOC);
        echo json_encode($projects);
    } catch (PDOException $e) {
        echo json_encode(['error' => 'Database error', 'message' => $e->getMessage()]);
...
function createProject($pdo) {
    try {
        ...
        // Query con colonne reali del database
        $stmt = $pdo->prepare("
            INSERT INTO projects (
                title, slug, description, short_description, content, image_url, 
                demo_url, github_url, status, is_featured, is_public, priority, 
